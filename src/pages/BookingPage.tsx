  ...
  useEffect(() => {
    fetchEvents().then(data => {
      const fallbackEvents = [
        { id: 1, title: '모로서기', date: '2026.05.16.(SAT)', time: '18:30', location: '서울예술대학교 중앙광장, 빨간대문', total_seats: 40, occupancy: 0 },
        { id: 2, title: '모로서기', date: '2026.05.17.(SUN)', time: '18:30', location: '서울예술대학교 중앙광장, 빨간대문', total_seats: 40, occupancy: 0 }
      ]
      const rawEvents = data.length > 0 ? data : fallbackEvents

      // Filter events to only keep today's date dynamically
      const today = new Date()
      ...
      let filteredEvents = rawEvents.filter(e => e.date.includes(todayStr))
      if (filteredEvents.length === 0) {
        // If today's date doesn't match any event (e.g. testing or viewing on a different day),
        // show all events loaded from Supabase without hardcoding.
